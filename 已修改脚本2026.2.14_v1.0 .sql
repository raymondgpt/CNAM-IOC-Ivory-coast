...
-- this table stores the opening date of each site
...
-- this table stores the opening date of each site
DROP VIEW IF EXISTS `dw`.`dw_site_enrollment`;
CREATE
...
DROP VIEW IF EXISTS `dm_enrollment_sites_total`;
CREATE
...
DROP VIEW IF EXISTS `dm_enrollment_sites_number_by_type`;
CREATE
...
DROP VIEW IF EXISTS `dm_enrollment_sites_number_by_region`;
CREATE
...
-- This table stores the opening date of each enrollment site by year
...
-- This table stores the opening date of each enrollment site by month
-- update 2026.02.14
drop view if exists dm_enrollment_sites_number_by_month;
create view dm_enrollment_sites_number_by_month as
select count(1)                                              as site_num,
       date_format(open_date, '%y-%m')                       as open_month,
       concat(m.month_fr, '-', date_format(open_date, '%y')) as eopen_month,
       date_format(open_date, '%y')                          as year
from `dw`.`dw_site_enrollment`
         join dim.dim_month_fr m
              on month(open_date) = m.month_num
group by concat(m.month_fr, '-', date_format(open_date, '%y')), date_format(open_date, '%y-%m'),
         date_format(open_date, '%y');


-- Indicateur NO.24 : Stock de cartes vierge préimprimées disponibles
-- this table stores the blank card stock of each site
DROP VIEW IF EXISTS dw.`dw_cartes_sites`;
CREATE
...
-- this table stores the blank card stock of each region
DROP VIEW IF EXISTS dm_available_blank_cards_by_region
CREATE
...
-- this table stores the total stock of blank card
DROP VIEW IF EXISTS dm_available_inventory_cards_total;
CREATE
...
--this table union the card production(in situ)&card production(prod masse) of each site in each date
...
-- this table records the stock-in and stock-out condition of the headquarter
...
-- this table records the card inventory of headquarter and each site
...
--   2. 分母：当月月初（1日）在职员工数量
--   3. 离职率 = 离职人数 / 月初在职人数
-- Indicateur NO.5: Modified employee turnover rate calculation logic
-- Operator Turnover Rate Report (Monthly)
-- Indicateur Definition: Calculate CMU reception staff turnover rate by time dimension (monthly)
-- Business Logic:
...
SELECT
    ...
    CONCAT(dmf.month_fr, '-', SUBSTRING(msc.month_key, 1, 4)) as month,
    COALESCE(departure.etl_time, active.etl_time, CURRENT_TIMESTAMP) as etl_time
FROM month_site_combinations msc
         LEFT JOIN `internal`.`dim`.`dim_month_fr` dmf
                   ON CAST(SUBSTRING(msc.month_key, 5, 2) AS TINYINT) = dmf.month_num
         LEFT JOIN (
...
SELECT
    ...
    CONCAT(dmf.month_fr, '-', SUBSTRING(mcc.month_key, 1, 4)) as month,
    COALESCE(departure.etl_time, active.etl_time, CURRENT_TIMESTAMP) as etl_time
FROM month_center_combinations mcc
         LEFT JOIN `internal`.`dim`.`dim_month_fr` dmf
                   ON CAST(SUBSTRING(mcc.month_key, 5, 2) AS TINYINT) = dmf.month_num
         LEFT JOIN (
    ...
    AND mcc.assigned_health_center = departure.assigned_health_center
         LEFT JOIN (
    -- Number of active employeess at the beginning of each month per assigned health center
    SELECT
        mcc.month_key,
...
---- this table records the card production of each day
-- update 2026.02.14
drop view if exists `dm`.`dm_average_card_production_per_day`;
create view `dm`.`dm_average_card_production_per_day` as
select period, eperiod, production_type, sum(value)
from dm.dm_production_declaratives_in_situ_and_prodmass_stock
group by period, eperiod, production_type
order by period, eperiod, production_type;
...
select a.region,
       ...
       concat(c.month_fr, '-', date_format(a.open_date, '%y')) as eopen_month,
       a.etl_time
from (select region,
             site_name,
             type,
             ...
             max(etl_time) as etl_time
      from `dw`.`dw_site_enrollment`
      group by region, site_name, type, open_date) a
         left join `dim`.`dim_country_district` b on
    a.region = b.name
         left join dim.dim_month_fr c on month(a.open_date) = c.month_num;


-- newly add 2026.02.14
-- 用来存放法语月份
create table dim.dim_month_fr (
                                  month_num tinyint,
                                  month_fr  string
)
    distributed by hash(month_num) buckets 1
properties ("replication_num"="1");
insert into dim.dim_month_fr values
